...
def learn(value):
    ...
    if implements_p_fit == 1:
        for i in list(range(5)):
            ...
            for i2 in list(range(20)):
                clf.partial_fit(shuffled_x[int(i2 * len(shuffled_x) / 20):int((i2 + 1) * len(shuffled_x) / 20)],
                                shuffled_y[int(i2 * len(shuffled_x) / 20):int((i2 + 1) * len(shuffled_x) / 20)]
                                , classes)
    else:
        database_indexs = list(range(len(labels)))
        random.shuffle(database_indexs)
