...
def rotate(img):
    r = cv2.getTrackbarPos("rot", MAIN_WINDOW_NAME)
    if len(img.shape) == 2:
        rows, cols = img.shape
    else:
        rows, cols, _ = img.shape

    M = cv2.getRotationMatrix2D((cols / 2, rows / 2), 90*r, 1)
...
def callback_depth(msg):
    ...
    try:
        ...
        img = rotate(img)  # added by François
...
# added by François BIDET
def getOrientation(ctn):
    ctn_clean = np.array(ctn[:,0,:], dtype=np.float32)
    # print(ctn_clean.shape)
    _, eigenvectors = cv2.PCACompute(ctn_clean, np.array([]))
    # print(eigenvectors)

    return str(math.atan2(eigenvectors[0][1], eigenvectors[0][0])) + " | " + str(math.atan2(eigenvectors[1][1], eigenvectors[1][0]))
...
def objects_detector(img_bgr8, i):
    ...
    print(str(getOrientation(cnt)))  # added by François BIDET
...
if __name__ == '__main__':
    ...
    cv2.createTrackbar("rot", MAIN_WINDOW_NAME, 0, 4, nothing)  # added by François BIDET
